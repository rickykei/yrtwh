...
var pos='pos3';
	 ...
	 function refresh() {
		...
		if (items != null) {
			...
			ul.append('<li>['+tmp+']' + data[2] +' / <a class="chgQty" data="'+index+'" dataDesc="'+data[2]+'" >'+ data[0] +'  件</a>($'+data[3]+') <a class="remove" data="'+index+'">X</a></li>');
			});
		 	ul.append('<li><b>總數: '+total+'</b></li>');
		}
		...
		var ino = localStorage.getItem('invoiceno');
		 ...
		 var inoh=$('#ino');
		  ...
		  if (ino!=null && ino!=''){
			...
			inoed.attr("href","/?page=invoice&subpage=invoice_edit.php&id="+ino);
			inoh.hide();
		  }else{
			  inoed.hide();
		  }
