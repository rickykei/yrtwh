...
(function($){
			...
			if ($('#'+id).length == 0) {
				...
				table.append(
					...
					$(options.rowTpl)
						...
						.append($(options.cellTpl).append($(options.buttonFunctionTpl).html(' ').addClass('sep').click(function(){
	...
	*/
	...
	$.fn.numpad.defaults = {
		target: false,
		openOnEvent: 'click',
		backgroundTpl: '<div class="ui-popup-screen ui-overlay-a"></div>',
		gridTpl: '<table class="ui-bar-a"></table>',
		displayTpl: '<input data-theme="b" type="text" />',
		displayCellTpl: '<td colspan="4"></td>',
		rowTpl: '<tr></tr>',
		cellTpl: '<td></td>',
		buttonNumberTpl: '<a data-role="button" data-theme="b"></a>',
		buttonFunctionTpl: '<a data-role="button" data-theme="a"></a>',
		gridTableClass: '',
