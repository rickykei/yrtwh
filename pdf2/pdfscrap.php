...
function Body($invoice_no)
{
   ...
   $result = $connection->query("SELECT * FROM scrap where invoice_no=".$invoice_no);
 ...
 $this->SetY(-3);
  ...
  if ($branchid=="A")  {
	...
	$this->Cell(40,8,iconv("UTF-8", "BIG5-HKSCS",$sales_name),$border,0,'R',0);
	...
	$this->Cell(40,8,iconv("UTF-8", "BIG5-HKSCS",$staffTel),$border,0,'R',0);
...
//   $this->Cell(216,8,iconv("UTF-8", "BIG5-HKSCS",$printShopDetail),$border,1,'C',0);
  ...
  $this->Cell(40,8,iconv("UTF-8", "BIG5-HKSCS",""),$border,1,'C',0);
	...
	$this->Cell(20,8,iconv("UTF-8", "BIG5-HKSCS","收貨人:"),$border,0,'R',0);
	$this->SetFont('Big5','',14);
	$this->Cell(90,8,iconv("UTF-8", "BIG5-HKSCS",$receiver),$border,0,'L',0);
	...
	$this->Cell(55,8,iconv("UTF-8", "BIG5-HKSCS",""),$border,0,'L',0);
	...
	$this->Cell(65,8,iconv("UTF-8", "BIG5-HKSCS",""),$border,0,'C',0);
...
//print goods_scrap
   $result = $connection->query("SET NAMES 'UTF8'");
   $result = $connection->query("SELECT goods_scrap.deductstock as deductstock, goods_scrap.cutting as cutting ,goods_scrap.goods_partno as goods_partno,goods_scrap.goods_detail as goods_detail,goods_scrap.qty as qty,goods_scrap.marketprice as marketprice,goods_scrap.manpower as manpower ,goods_scrap.subtotal as subtotal,sumgoods.goods_detail as goods_detail2 ,  discountrate,unit_name_chi FROM goods_scrap,sumgoods,unit where sumgoods.goods_partno=goods_scrap.goods_partno and invoice_no=".$invoice_no." and sumgoods.unitid=unit.id order by goods_scrap.id asc");
      if (DB::isError($result))
      die ($result->getMessage());
	  $i=1;
...
//	$this->Cell(22,6,iconv("UTF-8", "BIG5-HKSCS",$row2['manpower']),0,0,'C',0);
 ...
 $this->Cell(22,6,iconv("UTF-8", "BIG5-HKSCS",""),$border,1,'C',0);
	...
	$this->Cell(61,7,iconv("UTF-8", "BIG5-HKSCS","歡迎使用信用卡，免收手續費"),$border,1,'R',0);
...
$filepath='./scrap/pdf/'.$invoice_no.'.pdf';
if (file_exists($filepath)) {
	//move to backup
	$file_timestamp=date("Ymd_His") ;
	$filepathnew='./scrap/backuppdf/'.$invoice_no.'_'.$file_timestamp.'.pdf';
